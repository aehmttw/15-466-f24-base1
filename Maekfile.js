...
if (maek.OS === "windows") {
	...
	maek.options.LINKLibs.push(
		//linker flags for nest libraries:
		`-L${NEST_LIBS}/SDL3/lib`, `-lSDL3`, `-lm`,`-liconv`, `-framework`, `CoreAudio`, `-framework`, `AudioToolbox`, `-weak_framework`, `CoreHaptics`, `-weak_framework`, `GameController`, `-framework`, `ForceFeedback`, `-lobjc`, `-framework`, `CoreVideo`, `-framework`, `Cocoa`, `-framework`, `Carbon`, `-framework`, `IOKit`, `-framework`, `OpenGL`, //the output of sdl-config --static-libs
		`-L${NEST_LIBS}/libpng/lib`, `-lpng`,
		`-L${NEST_LIBS}/zlib/lib`, `-lz`
	);
}
...
if (maek.OS === 'windows') {
	copies.push(maek.COPY(`${NEST_LIBS}/SDL3/bin/SDL3.dll`, `dist/SDL3.dll`));
	copies.push(maek.COPY(`${NEST_LIBS}/SDL3/licenses/SDL3/LICENSE.txt`, `dist/SDL-LICENSE.txt`));
} else {
	copies.push(maek.COPY(`${NEST_LIBS}/SDL3/share/licenses/SDL3/LICENSE.txt`, `dist/SDL-LICENSE.txt`));
}
